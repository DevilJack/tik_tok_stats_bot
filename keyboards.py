...
def generate_inline_keyboard(args: ListOfButtons) -> InlineKeyboardMarkup:
    ...
    if not args.url:
        ...
        if not args.align:
            ...
            count = 0
            for row_size in args.align:
                keyboard.row(*[InlineKeyboardButton(text=str(text), url = str(url) , callback_data=str(callback_data))
                            for text, url, callback_data in
                            tuple(zip(args.text, args.url, args.callback))[count:count + row_size]])
                count += row_size
